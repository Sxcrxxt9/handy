...
type LoginVolunteerScreenProp = NativeStackNavigationProp<
  RootStackParamList,
  "LoginVolunteer"
>;
...
export default function LoginVolunteer() {
  ...
  const navigation = useNavigation<LoginVolunteerScreenProp>();
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (data.user?.type !== "volunteer") {
        Alert.alert("Alert", "This account is not registered as volunteer");
        return;
      }
      ...
      navigation.replace("MainVolunteer");
    } catch (error: any) {
      if (
  ...
  const handleRegister = () => {
    navigation.navigate("RegisterVolunteer" as never);
  };

  const handleForgotPassword = () => {
    navigation.navigate("ForgotVolunteer" as never);
  };
